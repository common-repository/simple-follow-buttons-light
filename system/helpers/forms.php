...
{
    // variables
    public $sfbl_checkboxes;

    // opening form tag
    public function open($wrap, $action = '', $class = '')
    {
        $return = '<div class="'.($wrap ? 'sfbl-form-wrap' : null).'">';
        $return .= '<form class="form-horizontal '.$class.'" id="sfbl-admin-form" method="post" action="'.$action.'">';

        // required hidden fields
        $return .= wp_nonce_field('sfbl_save_settings', 'sfbl_save_nonce');
        $return .= '<input type="hidden" name="sfbl_options" />';
    ...
    {
        // save button
        $return = '<button type="submit" id="submit" class="sfbl-btn-save btn btn-lg btn-primary"><i class="fa fa-floppy-o"></i></button>';

        // success button
        $return .= '<button type="button" class="sfbl-btn-save-success btn btn-lg btn-success"><i class="fa fa-check"></i></button>';
    ...
    // inline checkboxes
    public function sfbl_checkboxes($opts)
    {
        ...
        $input .= '<div class="">';
            ...
            // add all checkboxes
            array_walk($opts['checkboxes'], array($this, '_sfbl_add_checkboxes'));
        $input .= $this->sfbl_checkboxes;
    ...
    // checkboxes
    private function _sfbl_add_checkboxes($value, $key)
    {
        $this->sfbl_checkboxes .= '<label class="checkbox-inline no_indent">
									'.$key.'<br />
    ...
    public function sfbl_input($opts)
    {
        // check if opts passed is an array
        if (!is_array($opts)) {
            return 'Variable passed not an array';
        ...
        switch ($opts['type']) {
            ...
            case 'image_upload':
                $input .= '<div class="input-group">
						    ...
						    <span class="input-group-btn">
						      <button id="upload_'.$opts['name'].'_button" class="sfblUpload sfbl_upload_btn btn btn-default" data-sfbl-input="'.$opts['name'].'" type="button">Upload</button>
						    </span>
						  </div>';
            break;
            ...
            case 'colorpicker':
                ...
                $input .= '<input id="'.$opts['name'].'" name="'.$opts['name'].'" type="text" class="sfbl-colorpicker form-control" value="'.$opts['value'].'" placeholder="#4582ec" style="border-color: '.($opts['value'] != '' ? $opts['value'] : '#eaeaea').'" />';
            break;
